...
## Amendment, 2026-09-12: the age bound excludes a failing refresh, and where the read runs
...
- **The measurement now exists and the policy deliberately does not.**
  ...
  refusing is an availability-breaking policy, a sidecar part-way through rewriting a mounted key set
  would take a deployment's whole authentication down with it, and which way that default points is
  [#141](https://github.com/telekom/sutura/issues/141)'s decision - degradation per dependency -
  rather than the cache's. Until it is taken, the honest position is this paragraph plus a `WARN`
  carrying the staleness on every look that could not confirm the keys.
- **Where the read runs was also not what this record implied.** *"One small read per minute"* ran
  ...
  completing out of order unreachable rather than merely unlikely. **A blocking task still cannot be
  aborted**, so a look that never returns stops refresh for the life of the process; that is visible
  as a staleness that only grows, and it is the bounded end of the two.
