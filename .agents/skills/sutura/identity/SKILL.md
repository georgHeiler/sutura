...
- **That age bound EXCLUDES a failing refresh, and the exclusion is not small.** A read that fails
  ...
  deliberately no ceiling on it, because refusing is an availability-breaking policy and that
  default is #141's to point rather than the cache's. **Do not cite the age bound without the
  exclusion.**
