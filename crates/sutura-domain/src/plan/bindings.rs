...
//!
//! Two of the four dialects rendered for write a POSITIONAL placeholder - a bare `?` - so the Nth
//! placeholder in the statement takes the Nth value in the list. The renderer emits predicates in
//! filter order and the adapter binds the list in list order, so those two agree only while the
//! indices run `0, 1, .. n-1` down the filters. Read off the shipped adapters rather than reasoned
//! about: `sutura_exec_duckdb::bind` and `sutura_exec_postgres::bind` both map
//! [`QueryPlan::params`](crate::plan::QueryPlan::params) in list order, and `sutura_sql`'s
//! placeholder for that style ignores the position it is given.
//!
//! So a plan whose filters name the same parameters in a different order renders correctly on a
...
//! incoherent set**, because [`PlanBindings`] is the only way to supply one and [`Self::parse`] is
//! the only way to obtain one that holds a parameter - the argument
...
pub enum IncoherentBindings {
    ...
    ParamOutOfRange { index: usize, params: usize },
    ...
    pub fn parse(filters: Vec<PlanFilter>, params: Vec<ParamValue>) -> Result<Self, IncoherentBindings> {
        ...
        for filter in &filters {
            if let Some(index) = filter.predicate().param() {
                if index >= params.len() {
                    return Err(IncoherentBindings::ParamOutOfRange {
                        index,
                        params: params.len(),
                    });
                }
                if index != placeholder {
                    return Err(IncoherentBindings::ParamOutOfOrder {
        ...
        if placeholder != params.len() {
            return Err(IncoherentBindings::ParamNeverRead {
                read: placeholder,
                params: params.len(),
            });
        }
