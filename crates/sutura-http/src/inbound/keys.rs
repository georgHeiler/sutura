...
//! | how many looks are detached at once | `Cached::reading`, set in the same acquisition that reserves | a look that never returns, which stops refresh for the life of the process |
...
//!
//! # The revocation bound EXCLUDES a failing refresh, and the policy for that is not decided here
//!
...
//!
//! **The measurement is here and the policy is not.** `Cached::last_success` and
...
//! attempt. Nothing here refuses on it, because refusing is an availability-breaking policy: a
//! sidecar part-way through rewriting a mounted file would take a deployment's whole authentication
//! down with it. Which way that default points is `github.com/telekom/sutura#141`'s decision -
//! degradation per dependency - and until it is taken the honest position is the paragraph above
//! plus a log line carrying the staleness.
//!
...
struct Cached {
    ...
    /// Whether a look at the source is in flight.
    ///
    /// Set in the same acquisition that reserves, because the read now runs on the blocking pool and
    /// can outlast its own window. Without it a slow source accumulates one detached read per
    /// window, and two of them completing out of order install the OLDER document - [`adopt`]
    /// compares against the last bytes examined, so a late arrival looks like a change. One look at
    /// a time makes that ordering unrepresentable rather than checked.
    ///
    /// **The limit, stated because it is the cost of the choice:** a look that never returns never
    /// clears this, so refresh stops for the life of the process. That is the better of the two
    /// outcomes - a source that hangs once hangs again, and the alternative accumulates stuck pool
    /// threads - and it is visible as a [`KeySetCache::stale_for`] that only grows.
    reading: bool,
}
...
impl KeySetCache {
    ...
    /// documentation for what that permits and for whose decision the ceiling is. A rotation can
    ...
    ///
    /// **A look already in flight loses too, whatever the windows say.** `Cached::reading` is set
    /// here, under the same acquisition, because [`Self::look`] now runs on the blocking pool and can
    /// outlast its own window - so the window alone stopped bounding how many detached reads exist.
    /// See that field for the ordering hazard it removes and for what a look that never returns
    /// costs.
    async fn reserve(&self, now: Instant) -> bool {
        ...
        if cached.reading || now.saturating_duration_since(cached.last_attempt) < window {
            return false;
        }
    ...
    /// and what bounds how many looks exist is `Cached::reading`.
    ...
    async fn record(&self, now: Instant, looked: Looked) -> (Refreshed, Duration) {
        let mut cached = self.state.write().await;
        // The reservation is over, in ONE place and whatever the outcome. The only case that does
        // not reach it is a look that never returns - see `Cached::reading`.
        cached.reading = false;
    ...
    /// argues why that decision is not this file's to take. It is the number a freshness ceiling
    ...
    pub async fn poll_once(&self, now: Instant) -> Refreshed {
        ...
        // either. Unreadable is deliberately not "changed": a mounted secret being swapped can make
        // a path briefly absent, and treating that as a rotation would reject on every swap.
        ...
        let (outcome, stale_for) = self.record(now, looked).await;
